"""
This file carries out Contextual Bayesian Optimisation.
"""
# -------------------------------------------------------------------------------------------------------------------- #
...
# Private
from src.models.cbo_llm import CBOLLM
from src.models.bo_llm import BOLLM
    ...
    for i in range(1, N):
        ...
        if num_context < 2:
            ...
            bo_point = {key: value for key, value in
                        final_opt_points_dict["BO"]["BO-LLM"]["Optimal Point with MMR"][0][0].items() if
                        key not in {"Solubility", "Temperature"}}
            pools["BO"].pop(pools["BO"].index(bo_point))
            # C-BO point
            cbo_point = {key: value for key, value in
                         final_opt_points_dict["C-BO"]["CBO-LLM"]["Optimal Point with MMR"][0][0].items() if
